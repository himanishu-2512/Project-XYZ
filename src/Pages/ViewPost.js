...
const ViewPost = () => {
	...
	const handleShare = (id) => {
		navigator.clipboard.writeText(`https://master--adorable-trifle-92330c.netlify.app/post/${id}`);
	...
	const handleClickPost = (e, id) => {
		...
		if (title && caption) {
			axios.post(`${BASE_URL}/post/updatepost/${userId}/${id}`, updatedPost).then((res) => {
			...
			>
				...
				>
					<CardHeader
						...
						action={
							<IconButton aria-label="settings" onClick={(e) => handleAnchor(e, post._id)}>
								<MoreVert />
							</IconButton>
						}
